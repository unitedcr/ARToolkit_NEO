...
public class ARCamera : MonoBehaviour
{
    ...
    //Neo version Added
    private bool markerInsight = false;

    private bool gyroEnabled;
    private Gyroscope gyro;

    private Quaternion getfromgyro;
    ...
    void Start()
    {
        getfromgyro = Quaternion.identity;
        gyroEnabled = EnableGyro(ref gyro);
        rotFix = new Quaternion(0, 0, 1, 0);
    }

    void Update()
    {
        if (!markerInsight && gyroEnabled)
        {
            //getfromgyro = Quaternion.Euler(90.0f, 0.0f, 0.0f) * new Quaternion(gyro.attitude.x, gyro.attitude.y, -gyro.attitude.z, -gyro.attitude.w);
            getfromgyro = new Quaternion(gyro.attitude.x, gyro.attitude.y, -gyro.attitude.z, -gyro.attitude.w);
            arRotation = getfromgyro;
    ...
    //Neo version Added
    private bool EnableGyro(ref Gyroscope gyroscope)
    {
